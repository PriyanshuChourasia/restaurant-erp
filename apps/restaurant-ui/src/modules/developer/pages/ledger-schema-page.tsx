...
const SCHEMA: TableSchema[] = [
  {
    ...
    description: 'Chart of accounts — defines financial account types (Asset, Liability, Equity, Revenue, Expense)',
    columns: [
      { name: 'id', type: 'uuid PK', nullable: false, default: null, description: 'Primary key' },
      { name: 'name', type: 'varchar(255)', nullable: false, default: null, description: 'Account name' },
      { name: 'code', type: 'varchar(20)', nullable: true, default: null, description: 'Account code' },
      { name: 'account_type', type: 'enum(asset, liability, equity, revenue, expense)', nullable: false, default: null, description: 'Account classification' },
      { name: 'description', type: 'text', nullable: true, default: null, description: 'Optional description' },
      { name: 'opening_balance', type: 'decimal(14,2)', nullable: false, default: '0', description: 'Opening balance' },
      { name: 'current_balance', type: 'decimal(14,2)', nullable: false, default: '0', description: 'Current balance (running)' },
      { name: 'financial_year', type: 'varchar(20)', nullable: true, default: null, description: 'Financial year' },
    ...
    indices: [],
    relations: ['Referenced by ledger_entries.account_id'],
  },
  {
    table: 'ledger_entries',
    description: 'Transaction log — records debits and credits against ledger accounts',
    columns: [
      { name: 'id', type: 'uuid PK', nullable: false, default: null, description: 'Primary key' },
      { name: 'account_id', type: 'uuid FK', nullable: false, default: null, description: 'FK → ledger_accounts.id' },
      { name: 'entry_date', type: 'date', nullable: false, default: null, description: 'Entry date' },
      { name: 'type', type: 'enum(credit, debit)', nullable: false, default: null, description: 'Entry type (credit or debit)' },
      { name: 'amount', type: 'decimal(14,2)', nullable: false, default: null, description: 'Amount' },
      { name: 'description', type: 'text', nullable: true, default: null, description: 'Description' },
      { name: 'category', type: 'enum(sales, purchase, expense, salary, tax, miscellaneous)', nullable: false, default: "'miscellaneous'", description: 'Transaction category' },
      { name: 'reference', type: 'varchar(255)', nullable: true, default: null, description: 'Reference document' },
      { name: 'balance_after', type: 'decimal(14,2)', nullable: false, default: '0', description: 'Running balance after this entry' },
      { name: 'journal_entry_id', type: 'uuid', nullable: true, default: null, description: 'FK → journal_entries.id' },
      { name: 'created_by', type: 'uuid', nullable: true, default: null, description: 'User who created the entry' },
      { name: 'createdAt', type: 'timestamp', nullable: false, default: null, description: 'Auto-set creation timestamp' },
    ],
    indices: ['idx_ledger_account', 'idx_ledger_date'],
    relations: ['account_id → ledger_accounts.id', 'journal_entry_id → journal_entries.id'],
  },
  {
    table: 'journal_entries',
    description: 'Double-entry bookkeeping — tracks balanced journal entries with source references',
    columns: [
      { name: 'id', type: 'uuid PK', nullable: false, default: null, description: 'Primary key' },
      { name: 'entry_number', type: 'varchar(50) UNIQUE', nullable: false, default: null, description: 'Generated entry number' },
      { name: 'entry_date', type: 'date', nullable: false, default: null, description: 'Entry date' },
      { name: 'narration', type: 'text', nullable: true, default: null, description: 'Description / narration' },
      { name: 'source_type', type: 'enum(invoice, voucher, stock_movement, manual, reversal, credit_note)', nullable: false, default: null, description: 'Source system' },
      { name: 'source_id', type: 'uuid', nullable: true, default: null, description: 'Source document ID' },
      { name: 'reversal_of_id', type: 'uuid', nullable: true, default: null, description: 'Reversed entry reference' },
      { name: 'created_by', type: 'uuid', nullable: true, default: null, description: 'User who created the entry' },
      { name: 'createdAt', type: 'timestamp', nullable: false, default: null, description: 'Auto-set creation timestamp' },
    ],
    indices: ['idx_journal_entry_date', 'idx_journal_entry_source'],
    relations: ['Referenced by ledger_entries.journal_entry_id'],
  },
]

const TABLE_LINKS: Record<string, string> = {
  ledger_accounts: '/developer/tables/ledger_accounts',
  ledger_entries: '/developer/tables/ledger_entries',
  journal_entries: '/developer/tables/journal_entries',
}
...
export function LedgerSchemaPage() {
  return (
    <div className="max-w-5xl mx-auto space-y-5 sm:space-y-6">
      {/* Header */}
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div className="flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-br from-purple-500 to-violet-600 text-white shadow-md shadow-purple-500/20 shrink-0">
            <Database size={16} className="sm:hidden" />
            <Database size={20} className="hidden sm:block" />
          </div>
          <div className="min-w-0">
            <h1 className="text-base sm:text-xl font-bold text-gray-900 tracking-tight truncate">Ledger Schema</h1>
            <p className="text-[11px] sm:text-xs text-gray-400 truncate">3 tables · ledger_accounts → ledger_entries ← journal_entries</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
        ...
        <div className="flex flex-col items-start gap-1.5 sm:hidden">
          <ErdNode table="ledger_accounts" theme={THEME.ledger_accounts} />
          <div className="flex items-center gap-1.5 pl-3 text-[10px] text-gray-300 font-mono">
            <ChevronDown size={12} /> 1 ── N
          </div>
          <ErdNode table="ledger_entries" theme={THEME.ledger_entries} />
          <div className="flex items-center gap-1.5 pl-3 text-[10px] text-gray-300 font-mono">
            <ChevronDown size={12} /> N ── 1
          </div>
          <ErdNode table="journal_entries" theme={THEME.journal_entries} />
        </div>
          ...
          <div key={table.table} className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-2 px-3.5 sm:px-4 py-3 border-b border-gray-100 bg-gray-50">
              ...
              {link && (
                <Link
                  to={link}
                  className="flex items-center gap-0.5 text-[10px] sm:text-[11px] text-purple-600 hover:text-purple-800 font-medium shrink-0"
                >
      ...
      <div className="rounded-2xl border border-purple-200 bg-purple-50 p-4 sm:p-5">
        <p className="text-[11px] sm:text-xs text-purple-700 leading-relaxed">
          ...
          <code className="mx-1 text-[10px] sm:text-[11px] bg-purple-100 px-1 py-0.5 rounded">synchronize: true</code>
          (dev mode). No migration files are used. This schema is auto-generated from the entity classes in{' '}
          <code className="ml-1 text-[10px] sm:text-[11px] bg-purple-100 px-1 py-0.5 rounded break-all">apps/api/src/ledger/entities/</code>.
        </p>
      </div>
    </div>
  )
