...
const SCHEMA: TableSchema[] = [
  {
    ...
    description: 'Groups ledger accounts into logical sub-categories under each account nature (e.g., Current Assets, Operating Revenue, Direct Costs)',
    columns: [
      { name: 'id', type: 'uuid PK', nullable: false, default: null, description: 'Primary key' },
      { name: 'name', type: 'varchar(100) UNIQUE', nullable: false, default: null, description: 'Group name (e.g., Current Assets)' },
      { name: 'code', type: 'varchar(20) UNIQUE', nullable: false, default: null, description: 'Short code (e.g., CA, OR)' },
      { name: 'nature_id', type: 'uuid FK', nullable: false, default: null, description: 'FK → account_natures.id' },
    ...
    indices: ['idx_account_group_nature'],
    relations: ['nature_id → account_natures.id', 'Referenced by ledger_accounts.group_id'],
  },
]

const TABLE_LINKS: Record<string, string> = {
  account_groups: '/developer/tables/account_groups',
}
...
export function AccountGroupSchemaPage() {
  return (
    <div className="max-w-5xl mx-auto space-y-5 sm:space-y-6">
      {/* Header */}
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div className="flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-br from-cyan-500 to-teal-600 text-white shadow-md shadow-cyan-500/20 shrink-0">
            <Database size={16} className="sm:hidden" />
            <Database size={20} className="hidden sm:block" />
          </div>
          <div className="min-w-0">
            <h1 className="text-base sm:text-xl font-bold text-gray-900 tracking-tight truncate">Account Group Schema</h1>
            <p className="text-[11px] sm:text-xs text-gray-400 truncate">1 table · account_groups</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 font-mono text-xs font-semibold">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
            account_natures
          </span>
          <span className="text-gray-300 text-xs font-mono">1 ── N</span>
          <ChevronRight size={16} className="text-gray-300" />
          <ErdNode table="account_groups" theme={THEME.account_groups} />
          <span className="text-gray-300 text-xs font-mono">1 ── N</span>
          <ChevronRight size={16} className="text-gray-300" />
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-purple-200 bg-purple-50 text-purple-700 font-mono text-xs font-semibold">
          ...
          <div key={table.table} className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-2 px-3.5 sm:px-4 py-3 border-b border-gray-100 bg-gray-50">
              ...
              {link && (
                <Link
                  to={link}
                  className="flex items-center gap-0.5 text-[10px] sm:text-[11px] text-cyan-600 hover:text-cyan-800 font-medium shrink-0"
                >
      ...
      <div className="rounded-2xl border border-cyan-200 bg-cyan-50 p-4 sm:p-5">
        <p className="text-[11px] sm:text-xs text-cyan-700 leading-relaxed">
          <span className="font-semibold">Note:</span> Account groups provide a secondary classification layer between account natures and individual ledger accounts.{' '}
          Each group belongs to a nature via <code className="mx-1 text-[10px] sm:text-[11px] bg-cyan-100 px-1 py-0.5 rounded">nature_id</code> FK.{' '}
          For example, <strong>Current Assets</strong> (CA) belongs to the <strong>Asset</strong> nature.
        </p>
      </div>
    </div>
  )
