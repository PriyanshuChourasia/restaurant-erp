...
const SCHEMA: TableSchema[] = [
  {
    table: 'customers',
    description: 'Customer profiles — stores contact info, type classification, and price level assignment',
    columns: [
      { name: 'id', type: 'uuid PK', nullable: false, default: null, description: 'Primary key' },
      { name: 'name', type: 'varchar(255)', nullable: false, default: null, description: 'Customer name' },
      { name: 'phone', type: 'varchar(20) UNIQUE', nullable: false, default: null, description: 'Unique phone number' },
      { name: 'email', type: 'varchar(255)', nullable: true, default: null, description: 'Email address' },
      { name: 'gstin', type: 'varchar(20)', nullable: true, default: null, description: 'GSTIN (for B2B invoicing)' },
      { name: 'customer_type', type: 'varchar(50)', nullable: false, default: "'regular'", description: 'Type: regular, corporate, or staff' },
      { name: 'price_level_id', type: 'uuid FK', nullable: true, default: null, description: 'FK → price_levels.id (assigned pricing tier)' },
    ...
    indices: ['idx_customer_phone (unique)', 'idx_customer_active', 'idx_customer_type'],
    relations: ['price_level_id → price_levels.id', 'Referenced by orders.customer_id', 'Referenced by sales_invoices.customer_id'],
  },
]

const TABLE_LINKS: Record<string, string> = {
  customers: '/developer/tables/customers',
}
...
export function CustomerSchemaPage() {
  return (
    <div className="max-w-5xl mx-auto space-y-5 sm:space-y-6">
      {/* Header */}
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div className="flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-br from-sky-500 to-blue-600 text-white shadow-md shadow-sky-500/20 shrink-0">
            <Database size={16} className="sm:hidden" />
            <Database size={20} className="hidden sm:block" />
          </div>
          <div className="min-w-0">
            <h1 className="text-base sm:text-xl font-bold text-gray-900 tracking-tight truncate">Customer Schema</h1>
            <p className="text-[11px] sm:text-xs text-gray-400 truncate">1 table · customers</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <ErdNode table="customers" theme={THEME.customers} />
          <span className="text-gray-300 text-xs font-mono">N ── 1</span>
          <ChevronRight size={16} className="text-gray-300" />
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-teal-200 font-mono text-xs font-semibold text-teal-700 bg-teal-50">
          ...
          <div key={table.table} className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-2 px-3.5 sm:px-4 py-3 border-b border-gray-100 bg-gray-50">
              ...
              {link && (
                <Link
                  to={link}
                  className="flex items-center gap-0.5 text-[10px] sm:text-[11px] text-sky-600 hover:text-sky-800 font-medium shrink-0"
                >
      ...
      <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 sm:p-5">
        <p className="text-[11px] sm:text-xs text-sky-700 leading-relaxed">
          ...
          <code className="mx-1 text-[10px] sm:text-[11px] bg-sky-100 px-1 py-0.5 rounded">synchronize: true</code>
          (dev mode). No migration files are used. This schema is auto-generated from the entity classes in{' '}
          <code className="ml-1 text-[10px] sm:text-[11px] bg-sky-100 px-1 py-0.5 rounded break-all">apps/api/src/customers/entities/</code>.
        </p>
      </div>
    </div>
  )
