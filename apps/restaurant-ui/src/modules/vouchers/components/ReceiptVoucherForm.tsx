...
import { createReceiptVoucher, getLedgerAccountOptions } from '../api/vouchers.api'

interface ReceiptVoucherFormProps {
  onClose: () => void
}

export function ReceiptVoucherForm({ onClose }: ReceiptVoucherFormProps) {
  ...
  const mutation = useMutation({
    ...
    onError: (err: any) => {
      setError(err?.response?.data?.message || 'Failed to create receipt voucher')
    },
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutation.mutate({
      paymentMode,
      amount: parsedAmount,
      creditLines: [{ accountId, amount: parsedAmount, description: narration || undefined }],
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-base font-semibold text-gray-900">Receipt Voucher</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-gray-500 mb-1 block">Receive Into</label>
          ...
          <div>
            <label className="text-xs font-medium text-gray-500 mb-1 block">Source Account (Credit)</label>
            ...
            <p className="text-xs text-gray-400 mt-1">Use "Accounts Receivable" to record a customer clearing a credit-sale invoice.</p>
          </div>
          ...
          >
            {mutation.isPending ? 'Posting...' : 'Post Receipt Voucher'}
          </button>
        </form>
      </div>
    </div>
