...
export function EditItemPage({ itemId }: EditItemPageProps) {
  ...
  const [form, setForm] = useState({
    ...
    gstRate: 18, unit: 'piece', isVeg: true, isActive: true,
    categoryId: '', description: '',
  })

  // Sync form when item data loads
  useEffect(() => {
    if (!item) return
    setForm({
      name: item.name,
  ...
  if (!item) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <p className="text-lg font-medium text-gray-500">Item not found</p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2"><Tag size={22} className="text-primary" /> Edit Item</h1>
          <p className="text-sm text-gray-500">Update details for "{item.name}"</p>
        </div>
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); mutation.mutate() }} className="space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={mutation.isPending} className="btn btn-primary flex items-center gap-2">
            <Save size={16} /> {mutation.isPending ? 'Saving...' : 'Update Item'}
          </button>
